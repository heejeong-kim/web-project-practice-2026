...
const COL = {
  NUMBER: 1, CLASS_GROUP: 2, TEAM_NAME: 3,
...
function doGet(e) {
  try {
    ...
    let result;
    if (action === 'list') result = { success: true, teams: getTeams() };
    else if (action === 'register') result = registerTeam(params);
    else if (action === 'updateTeam') result = updateTeam(params);
    else if (action === 'updateWeek') result = updateWeek(params);
...
function updateWeek(data) {
  const lock=LockService.getScriptLock(); lock.waitLock(10000);
  try {
    const number=Number(data.number), week=Number(data.week);
    ...
    const sheet=getSheet_(), rowIndex=findTeamRow_(sheet,number);
    if (!rowIndex) throw new Error('팀을 찾을 수 없습니다.');
    sheet.getRange(rowIndex,week+7).setValue(String(data.checked)==='true'); SpreadsheetApp.flush();
    return { success:true };
  } finally { lock.releaseLock(); }
}
...
function validateTeamInput_(data, editingNumber) {
  ...
  for (let i=0;i<2;i+=1) if (!members[i].studentId||!members[i].name) throw new Error('팀원 '+(i+1)+'의 학번과 이름을 모두 입력해 주세요.');
  if ((members[2].studentId&&!members[2].name)||(!members[2].studentId&&members[2].name)) throw new Error('팀원 3은 학번과 이름을 모두 입력하거나 둘 다 비워 주세요.');
  const ids=members.filter(m=>m.studentId).map(m=>m.studentId);
  if (new Set(ids).size!==ids.length) throw new Error('같은 학번을 한 팀에 중복 등록할 수 없습니다.');
  const otherTeams=getTeams().filter(team=>Number(team.number)!==Number(editingNumber));
...
function getNextNumber_(sheet){const lastRow=sheet.getLastRow();if(lastRow<2)return 1;const values=sheet.getRange(2,COL.NUMBER,lastRow-1,1).getValues().flat();return values.reduce((max,value)=>Math.max(max,Number(value)||0),0)+1;}
function findTeamRow_(sheet,number){const lastRow=sheet.getLastRow();if(lastRow<2)return 0;const values=sheet.getRange(2,COL.NUMBER,lastRow-1,1).getValues();for(let i=0;i<values.length;i+=1)if(Number(values[i][0])===Number(number))return i+2;return 0;}
function formatDate_(value){if(!value)return '';if(Object.prototype.toString.call(value)==='[object Date]'&&!isNaN(value.getTime()))return Utilities.formatDate(value,Session.getScriptTimeZone()||'Asia/Seoul','yyyy-MM-dd HH:mm:ss');return String(value);}
function output_(payload,callback){const json=JSON.stringify(payload);const callbackName=String(callback||'').trim();const useJsonp=/^[A-Za-z_$][0-9A-Za-z_$.]*$/.test(callbackName);const body=useJsonp?callbackName+'('+json+');':json;return ContentService.createTextOutput(body).setMimeType(useJsonp?ContentService.MimeType.JAVASCRIPT:ContentService.MimeType.JSON);}
