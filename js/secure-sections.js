(() => {
  ...
  function findHeadingForPlaceholder(ph){
    ...
    while(n){ if(/^H[1-6]$/.test(n.tagName)) return n; n=n.previousElementSibling; }
    return null;
  }
  ...
  function showDialog(heading, section){
    ...
    overlay.querySelector('form').addEventListener('submit',async e=>{
      ...
      try{
        ...
        holder.remove();
        overlay.remove();
      }catch(_){ err.style.display='block'; input.select(); }
    });
    input.focus();
  }
  ...
  async function init(){
    ...
    const attach=()=>{
      sections.forEach(section=>{
        ...
        if(!heading) return;
        if(isUnlocked(section.id)){ try{sessionStorage.removeItem(accessKey(section.id))}catch{} }
        const btn=document.createElement('button');
        btn.type='button';
        btn.className='secure-section-cta';
        btn.textContent='[클릭]';
        btn.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();showDialog(heading,section)});
        heading.appendChild(btn);
        heading.style.cursor='pointer';
        heading.addEventListener('click',()=>showDialog(heading,section));
      });
    };
    attach();
    const content=document.querySelector('#lecture-content');
    if(content) new MutationObserver(attach).observe(content,{childList:true,subtree:true});
  }
