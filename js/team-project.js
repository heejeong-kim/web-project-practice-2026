(() => {
  ...
  async function saveTeam(event) {
    ...
    try {
      setSubmitting(true);
      const result = await jsonp({ action: editingNumber ? 'updateTeam' : 'register', number: editingNumber ? String(editingNumber) : '', classGroup, teamName, idea, track, notionUrl, studentId1: members[0].studentId, studentName1: members[0].name, studentId2: members[1].studentId, studentName2: members[1].name, studentId3: members[2].studentId, studentName3: members[2].name });
  ...
  function renderTeams() {
    ...
    if (!tableBody) return;
    tableBody.innerHTML = filtered.map(team => `<tr><td>${Number(team.number)}</td><td><span class="class-badge">${escapeHtml(team.classGroup)}</span></td><td class="team-name-cell"><button type="button" class="team-edit-button" data-edit-team="${Number(team.number)}">${escapeHtml(team.teamName)}</button></td><td class="idea-cell">${escapeHtml(team.idea || '-')}</td><td><span class="track-badge ${team.track === '심화' ? 'is-advanced' : ''}">${escapeHtml(team.track || '-')}</span></td><td class="notion-url-cell">${renderNotionLink(team.notionUrl || '', team.teamName || '')}</td><td class="member-cell">${renderMember(team.members?.[0])}</td><td class="member-cell">${renderMember(team.members?.[1])}</td><td class="member-cell">${renderMember(team.members?.[2])}</td></tr>`).join('');
  }
